    ...
    def transition(self):
        ...
        for each_ep in range(self.episodes):
            ...
            for step in range(self.steps):
                ...
                # size = (1, action dimension, number of atoms) - Why do we reshape a state in this way? TODO 
                # e.g. size = (1, 2, 51)
                action_prob, _ = self.actor_network.predict(
                    np.array(current_state).reshape((1, self.input_dim[0], self.input_dim[1]))) 

                # calculate action value (Q-value) - NOTE computes the expected value of the action at current_state 
